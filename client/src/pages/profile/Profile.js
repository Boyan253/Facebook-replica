...
export default function Profile({ posts, openModal, likePostHandler, dislikePostHandler, openFriendsModal }) {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      const response = await fetch(`https://future-server.onrender.com/users/${userId}`, {
        headers: { 'authorization': auth.payload }
      }).catch((err) => {
        console.log(err);
        navigate('/login')
      ...
      <div className="profile">
        ...
        <div className="profileRight">
          ...
          <div className="profileRightBottom">
            <Feed posts={post} user={user} likePostHandler={likePostHandler} dislikePostHandler={dislikePostHandler} />
